...
export default function GlobalRaviVoice(){
  const[open,setOpen]=useState(false);const[listening,setListening]=useState(false);const[text,setText]=useState('');const[result,setResult]=useState<Result|null>(null);const[busy,setBusy]=useState(false);const[message,setMessage]=useState('');const[source,setSource]=useState<'voice'|'text'>('text');const[executed,setExecuted]=useState(false)
  const[path,setPath]=useState('')
  useEffect(()=>setPath(window.location.pathname),[])
  if(path.startsWith('/geet'))return null

  async function interpret(input:string,nextSource:'voice'|'text'){
    const q=input.trim();if(!q)return;setSource(nextSource);setBusy(true);setExecuted(false);setMessage('Understanding…');setResult(null)
  ...
  async function executeFinance(){
    if(!result||result.domain!=='finance'||result.intent!=='add_finance_transaction')return
    const p=result.payload||{};const amount=Number(p.amount)
    if(!(amount>=0)){setMessage('I need a valid amount before I can record this.');return}
    setBusy(true);setMessage('Recording approved transaction…')
    try{
      const r=await fetch('/api/finance',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({kind:'transaction',type:p.type||'expense',amount,currency:String(p.currency||'NZD').toUpperCase(),baseAmountNzd:String(p.currency||'NZD').toUpperCase()==='NZD'?amount:undefined,category:p.category||'Other',merchant:p.merchant||'',date:p.date||undefined,notes:p.notes||'',source,confidence:result.confidence,riskLevel:result.riskLevel,aiActionId:result.actionId||undefined})})
      const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||'Could not record transaction')
      if(result.actionId)await fetch(`/api/intelligence/actions/${result.actionId}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:'executed'})})
      setExecuted(true);setMessage('Recorded. The ledger and audit trail are updated.')
    }catch(e){setMessage(e instanceof Error?e.message:'Could not record transaction')}finally{setBusy(false)}
  }
  const canRoute=result&&['finance','tasks','journal','family','projects'].includes(result.domain)
  const canExecuteFinance=result?.domain==='finance'&&result.intent==='add_finance_transaction'&&Number(result.payload?.amount)>=0

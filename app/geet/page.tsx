...
export default function GeetPage(){
  ...
  const[reminders,setReminders]=useState<Reminder[]>([]);const[showAll,setShowAll]=useState(false);const[formOpen,setFormOpen]=useState(false)
  ...
  const load=async()=>{const r=await fetch('/api/reminders',{cache:'no-store'});if(r.status===401){setAuth(false);return}if(r.ok){const d=await r.json();setReminders(d.reminders||[]);setAuth(true)}}
  ...
  async function save(e:FormEvent){e.preventDefault();if(!title.trim()||!date||!time)return;setWorking(true);setMessage('Saving…')
    ...
    if(r.ok){await load();setFormOpen(false);reset()}else{setMessage('Could not save. Please try again.')};setWorking(false)
  }

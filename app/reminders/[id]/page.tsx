...
// an invalid date ("Invalid Date" shown instead of the friendly date/time).
function normTime(t:string){const m=String(t||'').match(/^(\d{1,2}):(\d{2})/);return m?`${m[1].padStart(2,'0')}:${m[2]}`:'09:00'}
function calendarUrl(r:Reminder){const start=new Date(`${r.date}T${normTime(r.time||'09:00')}:00`);const end=new Date(start.getTime()+30*60*1000);const fmt=(d:Date)=>d.toISOString().replace(/[-:]/g,'').replace(/\.\d{3}Z$/,'Z');const p=new URLSearchParams({action:'TEMPLATE',text:r.title,dates:`${fmt(start)}/${fmt(end)}`,details:r.notes||'Created from Ravi OS',ctz:'Pacific/Auckland'});return`https://calendar.google.com/calendar/render?${p.toString()}`}
...
export default function ReminderDetail({params}:{params:Promise<{id:string}>}){
  ...
  const when=useMemo(()=>reminder?new Date(`${reminder.date}T${normTime(reminder.time)}:00`).toLocaleString('en-NZ',{dateStyle:'full',timeStyle:'short'}):'',[reminder])

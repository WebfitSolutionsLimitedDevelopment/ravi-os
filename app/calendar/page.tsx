...
import { ArrowLeft, Bell, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Clock3, ListChecks, Plus } from 'lucide-react'
...
type Item={id:string;kind:'reminder'|'task';title:string;date:string;time:string;subtitle:string;href:string;priority?:string;posterUrl?:string}
...
function shiftMonth(key:string,delta:number){const[y,m]=key.split('-').map(Number);const d=new Date(y,m-1+delta,1);return monthKey(d)}
...
export default function CalendarPage(){
  ...
  useEffect(()=>{(async()=>{try{const[rr,tr]=await Promise.all([fetch('/api/reminders',{cache:'no-store'}),fetch('/api/tasks',{cache:'no-store'})]);if(rr.ok){const d=await rr.json();setReminders(d.reminders||[])}if(tr.ok){const d=await tr.json();setTasks(d.tasks||[])}if(!rr.ok&&!tr.ok)setError('Calendar could not be loaded.')}catch{setError('Calendar could not be loaded.')}finally{setLoading(false)}})()},[])
  const items=useMemo<Item[]>(()=>{
    const reminderItems=reminders.map(r=>({id:r.id,kind:'reminder' as const,title:r.title,date:r.date,time:r.time||'',subtitle:r.notes||`Added by ${r.createdBy==='geet'?'Geet':'Ravi'}`,href:`/reminders/${r.id}`,posterUrl:r.posterUrl}))
    const taskItems=tasks.filter(t=>t.status==='open'&&t.dueDate).map(t=>({id:t.id,kind:'task' as const,title:t.title,date:t.dueDate,time:t.dueTime||'',subtitle:t.category||'Task',href:'/tasks',priority:t.priority}))
    return [...reminderItems,...taskItems].filter(i=>i.date.startsWith(month)).sort((a,b)=>stamp(a)-stamp(b))
  },[reminders,tasks,month])

...
export default function ReminderDetail({params}:{params:Promise<{id:string}>}){
  ...
  if(!reminder)return <main className={styles.shell}><div className={styles.card}><Link href="/reminders" className={styles.back}><ArrowLeft/>Reminders</Link><h1>Reminder not found</h1><p>This reminder may have been deleted or is unavailable.</p></div></main>
  return <main className={styles.shell}><section className={styles.card}><Link href="/reminders" className={styles.back}><ArrowLeft/>Reminders</Link><p className={styles.eyebrow}>SHARED REMINDER</p><h1>{reminder.title}</h1><div className={styles.when}>{when}</div>{reminder.notes&&<div className={styles.section}><span>Venue & address</span><p>{reminder.notes}</p></div>}{reminder.posterUrl?<div className={styles.section}><span>Poster</span><a href={reminder.posterUrl} target="_blank" rel="noreferrer" className={styles.posterLink}><img src={reminder.posterUrl} alt="Reminder poster"/></a><small>Tap the poster to view it full size.</small></div>:reminder.imageName?<div className={styles.missingPoster}><FileImage/><span><b>{reminder.imageName}</b><small>The original poster is not available for this older reminder.</small></span></div>:null}<div className={styles.section}><span>Shared by</span><p>{reminder.createdBy==='geet'?'Geet':'Ravi'}</p></div><a className={styles.calendar} href={calendarUrl(reminder)} target="_blank" rel="noreferrer"><CalendarPlus/>Add to Google Calendar</a></section></main>
}

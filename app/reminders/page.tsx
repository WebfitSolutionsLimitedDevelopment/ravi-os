...
export default function ReminderCentre(){
  ...
  const[working,setWorking]=useState(false);const[message,setMessage]=useState('');const[notificationStatus,setNotificationStatus]=useState('Not enabled')

  const load=async()=>{const r=await fetch('/api/reminders',{cache:'no-store'});if(r.ok){const d=await r.json();setReminders(d.reminders||[])}setLoading(false)}
  const migrateLocal=async()=>{let legacy:LegacyReminder[]=[];try{legacy=JSON.parse(localStorage.getItem(storageKey)||'[]')}catch{};if(!legacy.length)return;const current=await fetch('/api/reminders',{cache:'no-store'});if(!current.ok)return;const d=await current.json();const existing=new Set((d.reminders||[]).map((x:Reminder)=>x.legacyClientId).filter(Boolean));for(const old of legacy){const legacyClientId=String(old.id);if(existing.has(legacyClientId))continue;await fetch('/api/reminders',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title:old.title,notes:old.notes||'',date:old.date,time:old.time,dueAt:dueAt(old.date,old.time),source:old.source,imageName:old.imageName,imageDataUrl:old.imageDataUrl,legacyClientId})})}localStorage.setItem('ravi-os-reminders-migrated','1')}
  useEffect(()=>{if('Notification'in window)setNotificationStatus(Notification.permission==='granted'?'Enabled on this device':'Not enabled');(async()=>{await migrateLocal();await load()})()},[])
